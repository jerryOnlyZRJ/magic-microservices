...
```bash
$ git clone git@code.byted.org:cg/startkit.git  startkit
$ cd startkit
```


```bash
$ npm config set registry "http://bnpm.byted.org"    # set npm registry
...
login_token = raw_input('login_token: ')
...
url = 'http://127.0.0.1:3000/mock/api/user_login?' + data
...
|`__ANALYZE__`|True when `process.env.NODE_ENV` is `test`|
...
pm2 stop app.json --env production
```
