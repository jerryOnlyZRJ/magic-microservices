...
import { portalHtmlParser, ICustomDOMMatcher } from '@byted-cg/open-source-portal-utils';
...
class PuzzleManifestWebpackPlugin {
  ...
  getHook(
    ...
    const hook = ((HtmlWebpackPlugin.constructor as unknown) as HtmlWebpackPluginType).getHooks(
      compilation,
    ).beforeEmit;
    if (!hook) {
      if (!HtmlWebpackPlugin) {
